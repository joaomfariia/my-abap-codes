CLASS zjp_18_value_corresponding DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zjp_18_value_corresponding IMPLEMENTATION.
...
**********************************************************************
    ...
    out->write(  `-------------------------------------------------------------` ).
    out->write(  `Example 1: Connection and Connection Nested` ).

    out->write( data = connection
                name = `Source Structure: Connection`).
    out->write( data = connection_nested
                name = `Source Structure: Connection Nested`).
...
**********************************************************************
    ...
    out->write(  `-------------------------------------------------------------` ).
    out->write(  `Example 2: Value Operator` ).
